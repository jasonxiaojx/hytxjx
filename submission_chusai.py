...
class UserPolicy:
    ...
    def daily_decision(self,t):
        ...
        def allocation_rdc(inventory,replenish,t):
            ...
            # RDC可分配库存量
            # 改为向上取整。
            Q = dict(dm_invntry[dm_invntry['dc_id']==0]['stock_quantity'].groupby(\
                                        by=dm_invntry.item_sku_id,sort=False).sum() * 0.85)
            ...
            partial_param = functools.partial(run_by_sku,dm_invntry)
        ...
        def allocation_fdc_i(init_cr,limit,inventory,allocation_rdc_r,i):
            ...
            dm_invntry_fdc_t['flag_gm'] = dm_invntry_fdc_t.dist_type.apply(lambda x: 1 if x=='G' else 0) 
            dm_invntry_fdc_t['flag_nb'] = 1 - dm_invntry_fdc_t['flag_gm']
            
            dm_invntry_fdc_t['gamma_m'] = stats.gamma.stats(a=dm_invntry_fdc_t['para1'], scale=dm_invntry_fdc_t['para2'], moments='mvsk')[0]
            dm_invntry_fdc_t['nbinom_m'] = stats.nbinom.stats(dm_invntry_fdc_t['para1'], dm_invntry_fdc_t['para2'], moments='mvsk')[0]
            dm_invntry_fdc_t['nbinom_m'].fillna(0,inplace=True)            
            dm_invntry_fdc_t['exp_sales'] = dm_invntry_fdc_t['gamma_m'] * dm_invntry_fdc_t['flag_gm'] + \
                                    dm_invntry_fdc_t['nbinom_m'] * dm_invntry_fdc_t['flag_nb']
                
            # 设置一个20天目标库存，控制调拨节奏，不要全部都调过来了
            max_ti_days = 20
            dm_invntry_fdc_t['max_ti'] = dm_invntry_fdc_t['exp_sales'] * max_ti_days
            dm_invntry_fdc_t['max_transfer'] = (dm_invntry_fdc_t['max_ti'] - dm_invntry_fdc_t['stock_quantity'])
            dm_invntry_fdc_t['max_transfer'] = dm_invntry_fdc_t['max_transfer'].apply(lambda x: x if x>0 else 0)                
            ...
            dm_invntry_fdc_t['cost_adj'] = dm_invntry_fdc_t['stockout_cost']*abandon_rate[i-1]+0.01*(1-abandon_rate[i-1])
            ...
            k = 0
            while step_cr > 0.001:
                # 逻辑是：先按照step_cr=0.1增加，一直到遇到limit后停止，然后step_cr减半继续增加，直到步长减少到0.001
                # 由于这里看的是最大的cr，考虑到是(1-cr)按照成本成比例，因此最大的不会超过1
                # 从结果看，先达到上限的都是SKU数，打印结果看平均循环约30次减少到0.001以下。
                step_cr = step_cr/2
                while True:
                    k += 1
                    ...
                    #dm_invntry_fdc_t['cr'] = 1-dm_invntry_fdc_t['m_cost']/dm_invntry_fdc_t['stockout_cost']*(1-dm_invntry_fdc_t['m_cost_sku_cr'])

                    dm_invntry_fdc_t['cr'] = dm_invntry_fdc_t['cr'].apply(lambda x: 1 if x>1 else (0 if x<0 else x))

                    dm_invntry_fdc_t['gamma_q'] = stats.gamma.ppf(dm_invntry_fdc_t['cr'],a=dm_invntry_fdc_t['para1']*2,scale=dm_invntry_fdc_t['para2'])
                    dm_invntry_fdc_t['nbinom_q'] = stats.nbinom.ppf(dm_invntry_fdc_t['cr'],dm_invntry_fdc_t['para1']*2,dm_invntry_fdc_t['para2'])
                    dm_invntry_fdc_t['nbinom_q'].fillna(0,inplace=True)
                    dm_invntry_fdc_t['demand_q'] = dm_invntry_fdc_t['gamma_q'] * dm_invntry_fdc_t['flag_gm'] + dm_invntry_fdc_t['nbinom_q'] * dm_invntry_fdc_t['flag_nb']

                    dm_invntry_fdc_t['transfer_x'] = dm_invntry_fdc_t['demand_q'] - dm_invntry_fdc_t['stock_quantity']
                    dm_invntry_fdc_t['transfer_x'] = dm_invntry_fdc_t.transfer_x.apply(lambda x: x if x>0 else 0)
                    
                    # 再和最大目标库存对应的最大调拨量取Min
                    dm_invntry_fdc_t['transfer_x'] = dm_invntry_fdc_t[['transfer_x','max_transfer']].min(axis=1)                    
                    
                    # 四舍五入取整，再和可分配量取min，得到真正的调拨决策量
                    dm_invntry_fdc_t['transfer_x'] = np.round(dm_invntry_fdc_t['transfer_x'])                                     
                    dm_invntry_fdc_t['transfer_x'] = dm_invntry_fdc_t[['transfer_x','capacity_interger']].min(axis=1)

                    
                    # 这里不太明白？
#                     if k==1:
#                         dm_invntry_fdc_t['rank'] = dm_invntry_fdc_t[dm_invntry_fdc_t['transfer_x']>0]['cost_adj'].rank(method='first', ascending=False)
#                         #dm_invntry_fdc_t['rank'] = dm_invntry_fdc_t[dm_invntry_fdc_t['ofstock']>0]['stockout_cost'].rank(method='first', ascending=False)
#                         dm_invntry_fdc_t['flag'] = dm_invntry_fdc_t['rank'].apply(lambda x: 1 if x<=sku_lm else 0)
#                         dm_invntry_fdc_t['transfer_x'] = dm_invntry_fdc_t['transfer_x']*dm_invntry_fdc_t['flag']

                    sku_cnt_fdc_t = dm_invntry_fdc_t[dm_invntry_fdc_t['transfer_x']>0]['transfer_x'].count()
                    capacity_cnt_fdc_t = dm_invntry_fdc_t[dm_invntry_fdc_t['transfer_x']>0]['transfer_x'].sum()
                    ...
                    if (sku_cnt_fdc_t>sku_lm) or (capacity_cnt_fdc_t>cp_lm):
                        ...
                        m_cost_sku_cr -= step_cr
                        break
#             print('dc:',i,'step_cr:',step_cr,'m_cost_sku_cr:',m_cost_sku_cr,'k:',k,\
#                           'sku_cnt_fdc_t:',sku_cnt_fdc_t,'capacity_cnt_fdc_t:',capacity_cnt_fdc_t)
#             print('===============')
            
#             print('dc:',i ,dm_invntry_fdc_i[dm_invntry_fdc_i['transfer_x']>0]['transfer_x'].count())
#             print('dc:',i ,dm_invntry_fdc_i[dm_invntry_fdc_i['transfer_x']>0]['transfer_x'].sum())
...
#             print('===============')
            ...
            if (capacity_flag == True):
                ...
                dm_invntry_fdc_i['allocation'] = dm_invntry_fdc_i['transfer_x']
                dm_invntry_fdc_cp = dm_invntry_fdc_i.copy()
            else:
                k = 0
                dm_invntry_fdc_i['flag1'] = dm_invntry_fdc_i.transfer_x.apply(lambda x: 1 if x>0 else 0)
                ...
                dm_invntry_fdc_i['m_cost_cp'] = dm_invntry_fdc_i.cost_adj.max()
                dm_invntry_fdc_cp = dm_invntry_fdc_i.copy()
                #dm_invntry_fdc_i['m_cost_cp'] = dm_invntry_fdc_i[dm_invntry_fdc_i['flag1']==1].stockout_cost.max()
                ...
                while step_cr_cp > 0.0001:  # 这里要比前面那个小！很重要
                    ...
                    while True:
                        ...
                        #dm_invntry_fdc_i['cr'] = 1-dm_invntry_fdc_i['m_cost_cp']/dm_invntry_fdc_i['stockout_cost']*(1-dm_invntry_fdc_i['m_cost_sku_cr_cp'])

                        dm_invntry_fdc_i['cr_cp'] = dm_invntry_fdc_i['cr_cp'].apply(lambda x: 1 if x>1 else (0 if x<0 else x))

                        dm_invntry_fdc_i['gamma_q_cp'] = stats.gamma.ppf(dm_invntry_fdc_i['cr_cp'],a=dm_invntry_fdc_i['para1']*2,scale=dm_invntry_fdc_i['para2'])
                        dm_invntry_fdc_i['nbinom_q_cp'] = stats.nbinom.ppf(dm_invntry_fdc_i['cr_cp'],dm_invntry_fdc_i['para1']*2,dm_invntry_fdc_i['para2'])
                        dm_invntry_fdc_i['nbinom_q_cp'].fillna(0,inplace=True)
                        dm_invntry_fdc_i['demand_q_cp'] = dm_invntry_fdc_i['gamma_q_cp'] * dm_invntry_fdc_i['flag_gm'] + dm_invntry_fdc_i['nbinom_q_cp'] * dm_invntry_fdc_i['flag_nb']

                        dm_invntry_fdc_i['transfer_x_cp'] = dm_invntry_fdc_i['demand_q_cp'] - dm_invntry_fdc_i['stock_quantity']
                        dm_invntry_fdc_i['transfer_x_cp'] = dm_invntry_fdc_i.transfer_x_cp.apply(lambda x: x if x>0 else 0)
                        dm_invntry_fdc_i['transfer_x_cp'] = dm_invntry_fdc_i['transfer_x_cp'] * dm_invntry_fdc_i['flag1']
                        
                        # 再和最大目标库存对应的最大调拨量取Min
                        dm_invntry_fdc_i['transfer_x_cp'] = dm_invntry_fdc_i[['transfer_x_cp','max_transfer']].min(axis=1)             
                        
                        # 四舍五入取整
                        dm_invntry_fdc_i['transfer_x_cp'] = np.round(dm_invntry_fdc_i['transfer_x_cp'])
                        dm_invntry_fdc_i['transfer_x_cp'] = dm_invntry_fdc_i[['transfer_x_cp','capacity_interger']].min(axis=1)
#                         if k==1:
#                             dm_invntry_fdc_i['rank_cp'] = dm_invntry_fdc_i[dm_invntry_fdc_i['transfer_x_cp']>0]['cost_adj'].rank(method='first', ascending=False)
#                             #dm_invntry_fdc_i['rank_cp'] = dm_invntry_fdc_i[dm_invntry_fdc_i['ofstock_cp']>0]['stockout_cost'].rank(method='first', ascending=False)
#                             dm_invntry_fdc_i['flag_cp'] = dm_invntry_fdc_i['rank_cp'].apply(lambda x: 1 if x<=sku_lm else 0)
#                             dm_invntry_fdc_i['transfer_x_cp'] = dm_invntry_fdc_i['transfer_x_cp']*dm_invntry_fdc_i['flag_cp']

                        capacity_cnt_fdc_i = dm_invntry_fdc_i[dm_invntry_fdc_i['transfer_x_cp']>0]['transfer_x_cp'].sum()
                ...
                # print('dc:',i,'step_cr_cp:',step_cr_cp,'m_cost_sku_cr:',m_cost_sku_cr_cp,'k:',k,\
                #               'sku_cnt_fdc_t:',sku_cnt_fdc_t,'capacity_cnt_fdc_i:',capacity_cnt_fdc_i,\
                #                 'capacity_cnt_fdc_cp:',dm_invntry_fdc_cp[dm_invntry_fdc_cp['transfer_x_cp']>0]['transfer_x_cp'].sum())
                # print('===============')

                # print('dc:',i ,dm_invntry_fdc_cp[dm_invntry_fdc_cp['transfer_x']>0]['transfer_x'].count())
                # print('dc:',i ,dm_invntry_fdc_cp[dm_invntry_fdc_cp['transfer_x']>0]['transfer_x'].sum())

                # print('===============')
                
                # print('dc:',i,'flag1 sku total cap:',dm_invntry_fdc_i[dm_invntry_fdc_i['flag1']==1]['capacity_interger'].sum())
        ...
        # sta = distribution_r.groupby('dc_id')['daily_decision'].agg({'sku_cnt':lambda x: sum(x>0),'units_cnt':'sum'})
        # print(sta)
        
        # sta = allocation_fdc_r.groupby('dc_id')['allocation'].agg({'sku_cnt':lambda x: sum(x>0),'units_cnt':'sum'})
        # print(sta)  # 是一样的。说明第二步的结果没有超过第一步算的可分配量。
